    ...
    <div class="flex-1 flex max-w-[100vw]">
        ...
        <aside class="hidden lg:flex lg:flex-col w-64 bg-[#112b50] text-white border-r border-[#2f76aa]">
            <nav class="flex-1 flex flex-col p-6 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('student.achievements') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('student.achievements') ? 'bg-[#2f76aa] text-white' : 'text-neutral-300 hover:bg-[#2f76aa]/50 hover:text-white' }} transition-all duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                    <span class="font-semibold">Moje Osiągnięcia</span>
                </a>
                @endif
                @endauth
    ...
    <div id="mobileMenu" class="hidden lg:hidden fixed inset-0 bg-black bg-opacity-75 z-40">
        <div class="bg-[#112b50] h-full w-64 p-6">
            ...
            <nav class="space-y-2">
                ...
                @auth
                @if(auth()->user()->hasRole('student'))
                <a href="{{ route('student.achievements') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg {{ request()->routeIs('student.achievements') ? 'bg-[#2f76aa] text-white' : 'text-neutral-300 hover:bg-[#2f76aa]/50' }}">
                    <span class="font-semibold">Moje Osiągnięcia</span>
                </a>
                @endif
                @endauth
